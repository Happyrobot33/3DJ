...
namespace com.happyrobot33.holographicreprojector
{
    ...
    public abstract class CallbackUdonSharpBehaviour<EnumType>
        : CallbackUdonSharpBehaviourDataStorage
        where EnumType : Enum
    {
        ...
        )
        {
            ...
            for (int i = 0; i < methodNames.Length; i++)
            {
                ...
                if (!_callbackData[id].DataDictionary[behaviour].DataList.Contains(methodNames[i]))
                {
                    /* subscribedMethods.Add(
                        string.Format(
                            "{0}.{1}",
                            Logger.ColorizeScript(behaviour),
                            Logger.ColorizeFunction(behaviour, methodNames[i])
                        )
                    ); */
            ...
            /* if (subscribedMethods.Count > 0)
                Log(
                    LogLevel.Callback,
                    string.Format(
                        "Subscribed [{0}] to Callback [{1}]",
                        subscribedMethods.ToArray<string>().Join(", "),
                        Logger.ColorizeFunction(this, callbackID.ToString())
                    )
                );
            if (failedMethods.Count > 0)
                Log(
                    LogLevel.Warning,
                    string.Format(
                        "Failed to subscribe [{0}] to Callback [{1}] as they are already registered!",
                        failedMethods.ToArray<string>().Join(", "),
                        Logger.ColorizeFunction(this, callbackID.ToString())
                    )
                ); */
        ...
        )
        {
                ...
                if (data.ContainsKey(behaviour))
                {
                    ...
                    if (methods.Contains(methodName))
                    {
                        ...
                        /* Log(
                            LogLevel.Callback,
                            string.Format(
                                "Removed [{0}.{1}] from Callback [{2}] of [{3}]",
                                Logger.ColorizeScript(behaviour),
                                Logger.ColorizeFunction(behaviour, methodName),
                                Logger.ColorizeFunction(this, callbackID.ToString()),
                                Logger.ColorizeScript(this)
                            )
                        ); */
                        return true;
                    }
            ...
            /* Log(
                LogLevel.Warning,
                string.Format(
                    "Tried to remove [{0}.{1}] from [{2}] when it didnt exist!",
                    Logger.ColorizeScript(behaviour),
                    Logger.ColorizeFunction(behaviour, methodName),
                    Logger.ColorizeFunction(this, callbackID.ToString())
                )
            ...
            if (_callbackData.ContainsKey(id))
            {
                ...
                /* if (calledMethods.Count > 0)
                    Log(
                        LogLevel.Callback,
                        string.Format(
                            "Running [{0}] for Callback [{1}]",
                            calledMethods.ToArray<string>().Join(", "),
                            Logger.ColorizeFunction(this, callbackID.ToString())
                        )
                    );
                if (failedMethods.Count > 0)
                    Log(
                        LogLevel.Warning,
                        string.Format(
                            "Failed to run [{0}] for Callback [{1}] as they were null and have been removed",
                            failedMethods.ToArray<string>().Join(", "),
                            Logger.ColorizeFunction(this, callbackID.ToString())
                        )
                    ); */
